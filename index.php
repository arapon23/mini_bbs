...
if (isset($_SESSION['id']) && $_SESSION['time'] + 3600 > time()) {
  ...
  $member = $members->fetch();  // ログインしてるユーザの情報が、データベースから引出される
} else {
	header('Location: login.php');
	exit();
}

if (!empty($_POST)) {   // 投稿するボタンがクリックされた時
  if ($_POST['message'] !== '') {
...
$page = max($page, 1);  // $pageと1を比較して、1の方が大きい時は1とする

$counts = $db->query('SELECT COUNT(*) AS cnt FROM posts');
$cnt = $counts->fetch();  // メッセージの件数が入る
$maxPage = ceil($cnt['cnt'] / 5); // ceil: 少数を切り上げる
$page = min($page, $maxPage);  // $pageと$maxPageを比較して、$maxPageの方が小さい時は$maxPageとする

$start = ($page - 1) * 5;

$posts = $db->prepare('SELECT m.name, m.picture, p.* FROM members m, posts p WHERE m.id=p.member_id ORDER BY p.created DESC LIMIT ?,5');

$posts->bindParam(1, $start, PDO::PARAM_INT);  // 1番目に$startを指定するの意。 
$posts->execute();
